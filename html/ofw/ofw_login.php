...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (filter_var($user_input, FILTER_VALIDATE_EMAIL)) {
        $sql = "SELECT * FROM ofw_profile WHERE BINARY email='$user_input'";
    } else {
        $sql = "SELECT * FROM ofw_profile WHERE BINARY username='$user_input'";
    }
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            echo "<script> alert('Invalid username/email or password.');
                    window.location.href = '../combine_login.html';</script>";
        }
    } else {
