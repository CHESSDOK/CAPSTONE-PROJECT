...
async function loadDataset(inputData) {
  ...
  console.log("Data received for clustering:", data);
  ...
  const dataFlat = data.map(d => {
      if (typeof d.salary !== 'undefined') {
          return [parseFloat(d.salary)];  // Ensure salary is a number
      }
      throw new Error("Data item does not have a salary property");
  });

  console.log("Flattened data for clustering:", dataFlat);
  return dataFlat;
}
